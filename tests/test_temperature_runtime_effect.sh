...
OUT_DIR="/tmp/test_temperature_runtime_effect_$(date +%s)"
mkdir -p "$OUT_DIR"

echo "============================================================"
echo " ТЕСТ: более высокая температура должна усиливать распространение"
echo "============================================================"
...
create_simulation() {
  ...
  curl -s -X POST "$API_URL/api/simulations" \
    ...
    -d "{
      \"name\": \"Temperature runtime effect (${temperature})\",
      \"description\": \"Проверка runtime-влияния температуры воздуха\",
      \"graphType\": 0,
      \"gridWidth\": 25,
      \"gridHeight\": 25,
...
create_simulation 30 "$MID_JSON"
create_simulation 40 "$HIGH_JSON"
...
if [[ -z "$LOW_SIM_ID" || "$LOW_SIM_ID" == "null" ]]; then
  echo "❌ Не удалось создать LOW temperature simulation"
  cat "$LOW_JSON"
  exit 1
fi

if [[ -z "$MID_SIM_ID" || "$MID_SIM_ID" == "null" ]]; then
  echo "❌ Не удалось создать MID temperature simulation"
  cat "$MID_JSON"
  exit 1
fi

if [[ -z "$HIGH_SIM_ID" || "$HIGH_SIM_ID" == "null" ]]; then
  echo "❌ Не удалось создать HIGH temperature simulation"
  cat "$HIGH_JSON"
  exit 1
fi
...
print(f"LOW  temperature: area={low['area']:.0f}, burning={low['burning']}, burned={low['burned']}, affected={low['affected']}, running={low['running']}")
print(f"MID  temperature: area={mid['area']:.0f}, burning={mid['burning']}, burned={mid['burned']}, affected={mid['affected']}, running={mid['running']}")
print(f"HIGH temperature: area={high['area']:.0f}, burning={high['burning']}, burned={high['burned']}, affected={high['affected']}, running={high['running']}")

errors = []

if mid["area"] < low["area"]:
    errors.append(f"при средней температуре area должна быть >= низкой: {mid['area']} < {low['area']}")

if high["area"] < mid["area"]:
    errors.append(f"при высокой температуре area должна быть >= средней: {high['area']} < {mid['area']}")

if mid["affected"] < low["affected"]:
    errors.append(f"при средней температуре affected должно быть >= низкой: {mid['affected']} < {low['affected']}")

if high["affected"] < mid["affected"]:
    errors.append(f"при высокой температуре affected должно быть >= средней: {high['affected']} < {mid['affected']}")

if high["area"] <= low["area"] and high["affected"] <= low["affected"]:
    errors.append("высокая температура не усилила распространение по сравнению с низкой")

if errors:
    print("❌ Температура воздуха не показывает ожидаемого runtime-эффекта")
    for err in errors:
        print("   -", err)
    sys.exit(1)

print("✅ Более высокая температура воздуха усиливает распространение в полной симуляции")
PY
